...
export type RolloutLanguage = 'es' | 'fr' | 'de' | 'pt' | 'it' | 'pl' | 'ru' | 'tr' | 'id' | 'sv' | 'ms' | 'bg' | 'hi' | 'bn' | 'ja' | 'ko' | 'ar';
...
export const profiles: Record<RolloutLanguage, LanguageProfile> = {
  es: { generator: 'Generador de', converter: 'Convertidor de', formatter: 'Formateador de', free: 'gratis', slugPrefix: 'generador', joiner: 'de', intro: 'Crea borradores claros en espanol y revisa cada resultado antes de usarlo.', intent: 'Consulta local de utilidad con la misma intencion que la herramienta en ingles.', review: 'Revisa exactitud, originalidad, compatibilidad y reglas del proyecto antes de publicar.', faqTopics: ['ideas utiles', 'copiar resultado', 'revisar antes de usar'] },
  fr: { generator: 'Generateur de', converter: 'Convertisseur de', formatter: 'Formateur de', free: 'gratuit', slugPrefix: 'generateur', joiner: 'de', intro: 'Creez des brouillons clairs en francais et verifiez chaque resultat avant usage.', intent: 'Intention locale alignee avec la page anglaise.', review: 'Verifiez exactitude, originalite, compatibilite et regles du projet avant publication.', faqTopics: ['idees utiles', 'copier le resultat', 'verifier avant usage'] },
...
export function safetyFor(tool: RolloutToolSpec, profile: LanguageProfile): string {
  ...
  const localizedSafety: Record<RolloutLanguage, Record<'seo' | 'developer' | 'name' | 'text', string>> = {
    ...
    fr: {
      ...
      name: 'Verifiez originalite, sensibilite, droits et disponibilite avant un usage public.',
