...
async function auditBubbleTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: BUBBLE TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'bubble-text-generator');
  if (!tool) throw new Error('Bubble Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR BUBBLE TEXT GENERATOR ===');
}

auditBubbleTextGenerator();
