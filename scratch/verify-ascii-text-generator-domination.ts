...
async function auditAsciiTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: ASCII TEXT ART GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'ascii-text-generator');
  if (!tool) throw new Error('ASCII Text Art Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (Patorjk TAAG/FIGlet/ManyTools/CoolSymbol):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR ASCII TEXT ART GENERATOR ===');
}

auditAsciiTextGenerator();
