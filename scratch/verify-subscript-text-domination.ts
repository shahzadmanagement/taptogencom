...
async function auditSubscriptTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: SUBSCRIPT TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'small-text-generator' || t.slug === 'subscript-text-generator');
  if (!tool) throw new Error('Subscript Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR SUBSCRIPT TEXT GENERATOR ===');
}

auditSubscriptTextGenerator();
