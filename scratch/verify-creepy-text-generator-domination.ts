...
async function auditCreepyTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: CREEPY TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'creepy-text-generator');
  if (!tool) throw new Error('Creepy Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR CREEPY TEXT GENERATOR ===');
}

auditCreepyTextGenerator();
