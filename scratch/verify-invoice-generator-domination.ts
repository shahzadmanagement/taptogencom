...
async function auditInvoiceGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: INVOICE GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'invoice-generator');
  if (!tool) throw new Error('Invoice Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (Wave/FreshBooks/Zoho/Invoice Ninja):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR INVOICE GENERATOR ===');
}

auditInvoiceGenerator();
