...
async function auditDoubleStruckTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: DOUBLE-STRUCK TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'fancy-text-generator' || t.slug === 'unicode-text-generator' || t.slug === 'double-struck-text-generator');
  if (!tool) throw new Error('Double-Struck Text Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (LingoJam/YayText/CoolSymbol/Messletters):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR DOUBLE-STRUCK TEXT GENERATOR ===');
}

auditDoubleStruckTextGenerator();
