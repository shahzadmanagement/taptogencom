...
async function auditSmallTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: SMALL TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'small-text-generator');
  if (!tool) throw new Error('Small Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR SMALL TEXT GENERATOR ===');
}

auditSmallTextGenerator();
