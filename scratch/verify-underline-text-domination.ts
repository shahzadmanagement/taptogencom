...
async function auditUnderlineTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: UNDERLINE TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'underline-text-generator');
  if (!tool) throw new Error('Underline Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR UNDERLINE TEXT GENERATOR ===');
}

auditUnderlineTextGenerator();
