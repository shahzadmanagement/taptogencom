...
async function auditUsernameGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: USERNAME GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'username-generator');
  if (!tool) throw new Error('Username Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (SpinXO/NordPass/Namechk/Jimpix):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR USERNAME GENERATOR ===');
}

auditUsernameGenerator();
