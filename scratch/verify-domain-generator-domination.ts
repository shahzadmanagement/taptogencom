...
async function auditDomainNameGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: DOMAIN NAME GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'domain-name-generator');
  if (!tool) throw new Error('Domain Name Generator tool missing from data registry');

  console.log('1. Tool Definition & Core SEO Properties:');
  ...
  console.log('   - Privacy & Browser Execution Verified:', !!insights.privacyNotes);

  const comparison = getComparisonBlockForTool(tool);
  console.log('\n3. Google Top 10 6-Way Competitor Matrix:');
  console.log('   - Title:', comparison.title);
  console.log('   - TapToGen Method:', comparison.methodA);
  console.log('   - Competitor Method (GoDaddy/Namecheap/Shopify):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR DOMAIN NAME GENERATOR ===');
}

auditDomainNameGenerator();
