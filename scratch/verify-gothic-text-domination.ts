...
async function auditGothicTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: GOTHIC TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'fancy-text-generator' || t.slug === 'gothic-text-generator');
  if (!tool) throw new Error('Gothic Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR GOTHIC TEXT GENERATOR ===');
}

auditGothicTextGenerator();
