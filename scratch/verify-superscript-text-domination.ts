...
async function auditSuperscriptTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: SUPERSCRIPT TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'small-text-generator' || t.slug === 'superscript-text-generator');
  if (!tool) throw new Error('Superscript Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR SUPERSCRIPT TEXT GENERATOR ===');
}

auditSuperscriptTextGenerator();
