...
async function auditStrikethroughTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: STRIKETHROUGH TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'strikethrough-text-generator');
  if (!tool) throw new Error('Strikethrough Text Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (LingoJam/YayText/CoolSymbol/Messletters/IGFonts):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR STRIKETHROUGH TEXT GENERATOR ===');
}

auditStrikethroughTextGenerator();
