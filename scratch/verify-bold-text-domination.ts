...
async function auditBoldTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: BOLD TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'bold-text-generator');
  if (!tool) throw new Error('Bold Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR BOLD TEXT GENERATOR ===');
}

auditBoldTextGenerator();
