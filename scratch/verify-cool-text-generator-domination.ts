...
async function auditCoolTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: COOL TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'cool-text-generator');
  if (!tool) throw new Error('Cool Text Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (CoolText/LingoJam/YayText/CoolSymbol):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR COOL TEXT GENERATOR ===');
}

auditCoolTextGenerator();
