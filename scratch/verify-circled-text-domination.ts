...
async function auditCircledTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: CIRCLED TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'bubble-text-generator' || t.slug === 'circled-text-generator');
  if (!tool) throw new Error('Circled Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR CIRCLED TEXT GENERATOR ===');
}

auditCircledTextGenerator();
