...
async function auditTextToBinaryGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: TEXT TO BINARY CONVERTER ===\n');

  const tool = tools.find(t => t.slug === 'text-to-binary-generator');
  if (!tool) throw new Error('Text to Binary Converter tool missing from data registry');
  ...
  console.log('   - Competitor Method (RapidTables/ConvertBinary/Duplichecker/Cryptii):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR TEXT TO BINARY CONVERTER ===');
}

auditTextToBinaryGenerator();
