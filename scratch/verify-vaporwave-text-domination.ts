...
async function auditVaporwaveTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: VAPORWAVE TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'vaporwave-text-generator');
  if (!tool) throw new Error('Vaporwave Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR VAPORWAVE TEXT GENERATOR ===');
}

auditVaporwaveTextGenerator();
