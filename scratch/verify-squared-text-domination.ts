...
async function auditSquaredTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: SQUARED TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'fancy-text-generator' || t.slug === 'unicode-text-generator' || t.slug === 'squared-text-generator');
  if (!tool) throw new Error('Squared Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR SQUARED TEXT GENERATOR ===');
}

auditSquaredTextGenerator();
