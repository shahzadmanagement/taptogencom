...
async function auditBioGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: BIO GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'bio-generator');
  if (!tool) throw new Error('Bio Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (Ahrefs/Hootsuite/Copy.ai/Jasper):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR BIO GENERATOR ===');
}
