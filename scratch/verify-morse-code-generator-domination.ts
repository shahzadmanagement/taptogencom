...
async function auditMorseCodeGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: MORSE CODE GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'morse-code-generator');
  if (!tool) throw new Error('Morse Code Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (Cryptii/MorseCode.world/Scrambled Eggs/RapidTables):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR MORSE CODE GENERATOR ===');
}

auditMorseCodeGenerator();
