...
async function auditGlitchTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: GLITCH TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'glitch-text-generator');
  if (!tool) throw new Error('Glitch Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR GLITCH TEXT GENERATOR ===');
}

auditGlitchTextGenerator();
