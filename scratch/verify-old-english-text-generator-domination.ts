...
async function auditOldEnglishTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: OLD ENGLISH TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'old-english-text-generator');
  if (!tool) throw new Error('Old English Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR OLD ENGLISH TEXT GENERATOR ===');
}

auditOldEnglishTextGenerator();
