...
async function auditLeetTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: LEET SPEAK GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'leet-text-generator');
  if (!tool) throw new Error('Leet Speak Generator tool missing from data registry');
  ...
  console.log('   - Competitor Method (LingoJam/Universal Leet/Cryptii/YayText):', comparison.methodB);
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR LEET SPEAK GENERATOR ===');
}

auditLeetTextGenerator();
