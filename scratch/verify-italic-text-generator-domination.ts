...
async function auditItalicTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: ITALIC TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'italic-text-generator');
  if (!tool) throw new Error('Italic Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR ITALIC TEXT GENERATOR ===');
}

auditItalicTextGenerator();
