...
toolBlocks.forEach((block, idx) => {
  ...
  // Find quote end
  let endQuoteIdx = -1;
  const singleQ = block.indexOf("'");
  const doubleQ = block.indexOf('"');
  if (singleQ !== -1 && doubleQ !== -1) {
    endQuoteIdx = Math.min(singleQ, doubleQ);
  } else {
    endQuoteIdx = singleQ !== -1 ? singleQ : doubleQ;
  }
...
console.log(`Successfully re-formatted tools.ts v4 with ${cleanToolStrings.length} tools!`);
