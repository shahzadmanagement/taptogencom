...
async function auditUnicodeTextGenerator() {
  console.log('=== TOOL DOMINATION AUDIT: UNICODE TEXT GENERATOR ===\n');

  const tool = tools.find(t => t.slug === 'unicode-text-generator');
  if (!tool) throw new Error('Unicode Text Generator tool missing from data registry');
  ...
  console.log('\n=== TOOL DOMINATION AUDIT PASSED FOR UNICODE TEXT GENERATOR ===');
}

auditUnicodeTextGenerator();
